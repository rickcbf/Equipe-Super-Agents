...
//|   RickEA FIBO GRID - robo de retracao de Fibonacci com grid      |
//|                                                                  |
//|   1) Identifica a pernada (topo/fundo por fractal).              |
//|   2) Pernada de ALTA  -> zona de VENDA  entre 0% e 38.2% da      |
//|      retracao (vermelho, opacidade baixa, igual ao TOP/BOTTOM).  |
//|      Pernada de QUEDA -> zona de COMPRA entre 0% e 38.2%         |
//|      (verde, mesma opacidade).                                   |
//|   3) Dentro da zona vai armando o grid (espacamento em pontos).  |
//|      Ao chegar no 38.2% PARA de abrir ordens novas.              |
//|   4) Stop: se o preco voltar 23% da pernada PARA FORA do extremo |
//|      fecha tudo e nao reentra nessa pernada.                     |
//|   5) Take: preco medio da cesta + alvo em US$ ou em pontos -     |
//|      bateu o alvo, encerra TODAS as operacoes de uma vez.        |
//+------------------------------------------------------------------+
...
void Panel(const SLeg &leg,int cnt,double lots,double avg,double profit)
  {
   ...
   PanelLine(0,InpBrand+"  "+_Symbol,clrSilver);
   if(!leg.ok)
     {
   ...
   PanelLine(1,"Pernada: "+side+(IsBlocked(leg)?"  [BLOQUEADA]":""),base);
