//+------------------------------------------------------------------+
//|                       RickEA_FiboGrid_v4_Validade2026.mq5        |
   ...
   if(Expired())
     {
      ...
      if(open==0)
        {
         Alert(InpBrand,": a validade deste robo terminou em 31/12/2026. Fale com @ri.chartrader para renovar.");
         return(INIT_FAILED);
        }
...
void OnTick()
  {
   ...
   //--- validade: depois de 31/12/2026 nao abre ciclo novo
   if(Expired())
     {
      if(!g_expiredMsg)
        {
         g_expiredMsg=true;
         Alert(InpBrand,": a validade terminou em 31/12/2026. Nenhuma ordem nova sera aberta. Fale com @ri.chartrader para renovar.");
        }
      if(cnt==0 && !g_closing)
...
string ExpiryText()
  {
   if(Expired())
      return("LICENCA EXPIRADA em 31/12/2026 - sem ordens novas");
   int days=(int)((LIC_EXPIRY-ServerNow())/86400);
   if(days<=15)
      return("Valido ate 31/12/2026 - faltam "+IntegerToString(days)+" dia(s)");
   return("Valido ate 31/12/2026");
  }
//+------------------------------------------------------------------+
