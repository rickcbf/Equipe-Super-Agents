...
#property description "v4: Fibo automatica ou MANUAL (arraste os pontos no grafico)"
...
enum ENUM_FIBOMODE
  {
   FIBO_AUTO=0,      // Automatica (o robo acha a pernada)
   FIBO_MANUAL=1     // Manual (voce arrasta a Fibo no grafico)
  };

//--- deteccao da pernada
input string InpBrand          = "RICKEA FIBO GRID v4"; // Titulo / marca
input ENUM_TRADEDIR InpTradeDir = DIR_BOTH; // Direcao: compras, vendas ou ambas
input ENUM_FIBOMODE InpFiboMode = FIBO_MANUAL; // Fibo: automatica ou manual (arrastar)
input bool   InpAutoButton     = true;     // Botao "FIBO AUTO" (reposiciona a Fibo manual na pernada automatica)
input bool   InpSyncSL         = true;     // Manual: mover o SL das ordens abertas quando a Fibo for movida
...
string   MFIB="FGM_FIBO";       // objeto da Fibo manual (prefixo proprio: sobrevive a troca de tempo grafico)
int      g_cycleDir=0;          // direcao do ciclo aberto (+1 vendido / -1 comprado)
bool     g_dirMismatch=false;   // Fibo manual invertida em relacao ao ciclo aberto
double   g_blockBasePx=0.0;     // precos da pernada bloqueada (modo manual)
double   g_blockExtPx=0.0;
double   g_lastSL=0.0;          // ultimo SL sincronizado nas ordens abertas
...
void   CreateManual();
void   PlaceManual(const SLeg &leg);
void   StyleManual(int dir);
void   SnapManualToAuto();
void   AutoButton();
void   SyncSL(double stop);
void   Refresh();
string DirName();
...
int OnInit()
  {
   ...
   if(!AccountOk())
     {
      Alert(InpBrand,": esta licenca e da conta ",IntegerToString(LIC_CONTA),
   ...
   if(Expired())
     {
      ...
      if(open==0)
        {
         Alert(InpBrand,": a validade deste robo terminou em ",LicDate(),". Fale com @ri.chartrader para renovar.");
         return(INIT_FAILED);
        }
      Print(InpBrand,": validade encerrada - o robo so vai administrar as ",open," ordem(ns) aberta(s), sem abrir novas.");
     }
   g_pt=_Point;
   ...
   g_leg.ok=false;
   g_lock.ok=false;
   if(InpFiboMode==FIBO_MANUAL)
     {
      ChartSetInteger(0,CHART_EVENT_OBJECT_DELETE,true);   // avisa se a Fibo for apagada
      CreateManual();
      if(InpAutoButton)
         AutoButton();
      Print(InpBrand,": modo MANUAL - arraste os dois pontos da Fibo (100% = origem, 0% = extremo).");
     }
...
//+------------------------------------------------------------------+
//| Eventos do grafico: arrastar a Fibo e botao FIBO AUTO             |
//+------------------------------------------------------------------+
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
  {
   if(InpFiboMode!=FIBO_MANUAL)
      return;
   if(id==CHARTEVENT_OBJECT_CLICK && sparam==PFX+"BTN_AUTO")
     {
   ...
   if(id==CHARTEVENT_OBJECT_DELETE && sparam==MFIB)
     {
      CreateManual();                  // apagou sem querer: recria na pernada automatica
      Refresh();
     }
  }
//+------------------------------------------------------------------+
void OnTick()
  {
   ...
   if(cnt==0)
      g_closing=false;
   //--- pernada: manual (lida do objeto a cada tick) ou automatica
   g_dirMismatch=false;
   if(InpFiboMode==FIBO_MANUAL)
     {
      if(ObjectFind(0,MFIB)<0)
         CreateManual();
      ReadManual(g_leg);
      if(g_active && g_leg.ok)
        {
         if(g_cycleDir==0 || g_leg.dir==g_cycleDir)
           {
            g_lock=g_leg;                    // Fibo movida: zona e stop acompanham
            if(InpAttachSL && InpSyncSL && cnt>0)
               SyncSL(StopPrice(g_lock));
           }
         else
            g_dirMismatch=true;              // Fibo invertida com ciclo aberto: mantem a anterior
        }
     }
   else
   ...
   //--- validade: depois do vencimento nao abre ciclo novo
   if(Expired())
     {
      if(!g_expiredMsg)
        {
         g_expiredMsg=true;
         Alert(InpBrand,": a validade terminou em ",LicDate(),". Nenhuma ordem nova sera aberta. Fale com @ri.chartrader para renovar.");
        }
      if(cnt==0 && !g_closing)
   ...
   if(!DirAllowed(leg.dir))                 // filtro: somente compras / somente vendas
      return;
   if(g_dirMismatch)                        // Fibo manual invertida com ciclo aberto
      return;
...
void AdoptPositions(int cnt)
  {
   ...
   SLeg leg;
   leg.ok=false;
   bool got=(InpFiboMode==FIBO_MANUAL)?ReadManual(leg):FindLeg(leg);
   if(got && leg.dir==dir)
      g_lock=leg;
   else
     {
...
void BlockLeg(const SLeg &leg)
  {
   ...
   g_blockDir=leg.dir;
   g_blockBasePx=leg.base;
   g_blockExtPx=leg.ext;
  }
//+------------------------------------------------------------------+
bool IsBlocked(const SLeg &leg)
  {
   if(g_blockDir==0)
      return(false);
   if(InpFiboMode==FIBO_MANUAL)   // manual: libera assim que voce mover a Fibo
      return(leg.dir==g_blockDir &&
             MathAbs(leg.base-g_blockBasePx)<g_pt/2.0 &&
             MathAbs(leg.ext-g_blockExtPx)<g_pt/2.0);
...
string ExpiryText()
  {
   if(Expired())
      return("LICENCA EXPIRADA em "+LicDate()+" - sem ordens novas");
   int days=(int)((LIC_EXPIRY-ServerNow())/86400);
   if(days<=15)
      return("Licenca "+LIC_CLIENTE+" - valida ate "+LicDate()+" - faltam "+IntegerToString(days)+" dia(s)");
   return("Licenca "+LIC_CLIENTE+" - valida ate "+LicDate());
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Le a Fibo manual: ponto 1 = origem (100%), ponto 2 = extremo (0%) |
//+------------------------------------------------------------------+
bool ReadManual(SLeg &leg)
  {
   leg.ok=false;
   ...
   if(p0<=0.0 || p1<=0.0 || MathAbs(p1-p0)<10*g_pt)
      return(false);
   leg.base =p0;
   leg.tBase=t0;
   leg.ext  =p1;
...
//+------------------------------------------------------------------+
//| Cria a Fibo manual (se ainda nao existe) na pernada automatica    |
//+------------------------------------------------------------------+
void CreateManual()
  {
   if(ObjectFind(0,MFIB)>=0)
   ...
   if(!FindLeg(a))
     {
      //--- sem pernada automatica: usa a maxima e a minima das ultimas 100 barras
      int hiIdx=iHighest(_Symbol,_Period,MODE_HIGH,100,1);
      int loIdx=iLowest(_Symbol,_Period,MODE_LOW,100,1);
      if(hiIdx<0 || loIdx<0)
         return;
      bool upLeg=(hiIdx<loIdx);        // maxima mais recente que a minima = pernada de alta
      a.base =upLeg?iLow(_Symbol,_Period,loIdx):iHigh(_Symbol,_Period,hiIdx);
      a.tBase=upLeg?iTime(_Symbol,_Period,loIdx):iTime(_Symbol,_Period,hiIdx);
      a.ext  =upLeg?iHigh(_Symbol,_Period,hiIdx):iLow(_Symbol,_Period,loIdx);
      a.tExt =upLeg?iTime(_Symbol,_Period,hiIdx):iTime(_Symbol,_Period,loIdx);
      a.dir  =upLeg?1:-1;
      a.ok   =(a.base>0.0 && a.ext>0.0 && a.base!=a.ext);
      if(!a.ok)
         return;
     }
   if(!ObjectCreate(0,MFIB,OBJ_FIBO,0,a.tBase,a.base,a.tExt,a.ext))
     {
      Print(InpBrand,": nao foi possivel criar a Fibo manual (",GetLastError(),").");
      return;
     }
   ...
   ObjectSetString(0,MFIB,OBJPROP_TOOLTIP,"Fibo do "+InpBrand+": arraste os pontos");
   double pcts[6]={0.0,23.6,38.2,50.0,61.8,100.0};
   ObjectSetInteger(0,MFIB,OBJPROP_LEVELS,6);
   for(int i=0; i<6; i++)
     {
      ObjectSetDouble(0,MFIB,OBJPROP_LEVELVALUE,i,pcts[i]/100.0);
      ObjectSetString(0,MFIB,OBJPROP_LEVELTEXT,i,DoubleToString(pcts[i],1)+"%  %$");
      ObjectSetInteger(0,MFIB,OBJPROP_LEVELSTYLE,i,(pcts[i]==0.0 || pcts[i]==100.0)?STYLE_SOLID:STYLE_DOT);
      ObjectSetInteger(0,MFIB,OBJPROP_LEVELWIDTH,i,1);
     }
   StyleManual(a.dir);
  }
//+------------------------------------------------------------------+
//| Move os dois pontos da Fibo manual                                |
//+------------------------------------------------------------------+
void PlaceManual(const SLeg &leg)
  {
   if(ObjectFind(0,MFIB)<0)
      return;
   ObjectMove(0,MFIB,0,leg.tBase,leg.base);
   ObjectMove(0,MFIB,1,leg.tExt,leg.ext);
   StyleManual(leg.dir);
  }
//+------------------------------------------------------------------+
//| Cor da Fibo manual pela direcao (vermelho venda / verde compra)   |
//+------------------------------------------------------------------+
void StyleManual(int dir)
  {
   if(ObjectFind(0,MFIB)<0)
      return;
   color clr=(dir==1)?InpSellClr:InpBuyClr;
   if(!DirAllowed(dir))
      clr=clrGray;
   ObjectSetInteger(0,MFIB,OBJPROP_COLOR,clr);
   int n=(int)ObjectGetInteger(0,MFIB,OBJPROP_LEVELS);
   for(int i=0; i<n; i++)
      ObjectSetInteger(0,MFIB,OBJPROP_LEVELCOLOR,i,clr);
  }
//+------------------------------------------------------------------+
//| Botao FIBO AUTO: leva a Fibo manual para a pernada automatica     |
//+------------------------------------------------------------------+
void SnapManualToAuto()
  {
   SLeg a;
   ...
   if(!FindLeg(a))
     {
      Print(InpBrand,": nenhuma pernada automatica encontrada agora - a Fibo manual ficou onde estava.");
      return;
     }
   if(ObjectFind(0,MFIB)<0)
      CreateManual();
   PlaceManual(a);
   Print(InpBrand,": Fibo manual reposicionada na pernada automatica.");
  }
//+------------------------------------------------------------------+
void AutoButton()
  {
   string n=PFX+"BTN_AUTO";
   if(ObjectFind(0,n)<0)
     {
...
//+------------------------------------------------------------------+
//| Leva o SL das ordens abertas para o novo stop da Fibo             |
//+------------------------------------------------------------------+
void SyncSL(double stop)
  {
   stop=NormalizeDouble(stop,_Digits);
   if(MathAbs(stop-g_lastSL)<g_pt/2.0)
      return;
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   double minDist=(double)SymbolInfoInteger(_Symbol,SYMBOL_TRADE_STOPS_LEVEL)*g_pt;
   bool allOk=true;
   for(int i=PositionsTotal()-1; i>=0; i--)
     {
      ulong tk=PositionGetTicket(i);
      ...
      double ref=isSell?ask:bid;
      bool rightSide=isSell?(stop>ref):(stop<ref);
      if(!rightSide || MathAbs(stop-ref)<=minDist)
        {
         allOk=false;                  // stop do lado errado ou perto demais: o robo fecha pelo stop da Fibo
         continue;
        }
      if(MathAbs(PositionGetDouble(POSITION_SL)-stop)<g_pt/2.0)
         continue;
      if(!trade.PositionModify(tk,stop,PositionGetDouble(POSITION_TP)))
        {
         allOk=false;
         Print(InpBrand,": falha ao mover SL #",tk," (",trade.ResultRetcode(),") ",trade.ResultRetcodeDescription());
        }
     }
   if(allOk)
      g_lastSL=stop;
...
//+------------------------------------------------------------------+
//| Redesenha na hora (quando voce arrasta a Fibo)                    |
//+------------------------------------------------------------------+
void Refresh()
  {
   int    cnt=0;
...
void DrawFibo(const SLeg &leg)
  {
   ...
   if(InpFiboMode==FIBO_MANUAL)
     {
      StyleManual(leg.dir);          // a Fibo manual ja mostra niveis e pernada
...
void Panel(const SLeg &leg,int cnt,double lots,double avg,double profit)
  {
   ...
   int daysLeft=(int)((LIC_EXPIRY-ServerNow())/86400);
   PanelLine(6,ExpiryText(),Expired()?clrRed:(daysLeft<=15?clrOrange:clrGray));
   if(!leg.ok)
     {
      if(InpFiboMode==FIBO_MANUAL)
         PanelLine(1,"Fibo MANUAL: afaste os dois pontos da Fibo (pernada pequena demais)",clrSilver);
      else
   ...
   if(g_dirMismatch)
      flag=flag+"  [FIBO INVERTIDA: feche o ciclo ou volte a Fibo]";
   PanelLine(1,(InpFiboMode==FIBO_MANUAL?"Fibo MANUAL: ":"Pernada: ")+side+flag,base);
